...
if st.sidebar.button('Show Data'):
	...
	new_start_date_df = pd.read_sql(f'''select datum from posrecon where RekNr = "{reknr}" order by datum asc limit 1;''', con = engine)
	new_end_date_df = pd.read_sql(f'''select datum from posrecon where RekNr = "{reknr}" order by datum desc limit 1;''', con = engine)
	
	new_start_date = pd.to_datetime(new_start_date_df['Datum'][0]).strftime("%Y-%m-%d")
	new_end_date = pd.to_datetime(new_end_date_df['Datum'][0]).strftime("%Y-%m-%d")
